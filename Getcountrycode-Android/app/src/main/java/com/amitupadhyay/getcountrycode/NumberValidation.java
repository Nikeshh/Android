...
public class NumberValidation extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        String accesscode = phonenumber.substring(0, Math.min(phonenumber.length(), 2));
        String providercode = phonenumber.substring(2, Math.min(phonenumber.length(), 5));
        String subscribernumber = phonenumber.substring(5, Math.min(phonenumber.length(), 11));
        Toast.makeText(this,"Access code:"+accesscode,Toast.LENGTH_SHORT).show();
        if(accesscode.equals("92")){
            Toast.makeText(this, "Operator:Tata Indicom CDMA", Toast.LENGTH_SHORT).show();
        }
        if(accesscode.equals("93")){
            Toast.makeText(this, "Operator:Reliance CDMA", Toast.LENGTH_SHORT).show();
        }
        if(accesscode.equals("94")){
            Toast.makeText(this, "Operator:BSNL GSM", Toast.LENGTH_SHORT).show();
        }
        Toast.makeText(this,"Provider code:"+providercode,Toast.LENGTH_SHORT).show();
        Toast.makeText(this,"Subscriber number:"+subscribernumber,Toast.LENGTH_SHORT).show();
