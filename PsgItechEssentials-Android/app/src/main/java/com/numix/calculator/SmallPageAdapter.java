...
public class SmallPageAdapter extends CalculatorPageAdapter {
    ...
    public View getViewAt(int position) {
        if(position == Calculator.SmallPanel.FUNCTION.getOrder() && CalculatorSettings.functionPanel(mParent.getContext())) {
            return mFunctionPage;
        }
        else if(position == Calculator.SmallPanel.ADVANCED.getOrder() && CalculatorSettings.advancedPanel(mParent.getContext())) {
            return mAdvancedPage;
        }
        else if(position == Calculator.SmallPanel.HEX.getOrder() && CalculatorSettings.hexPanel(mParent.getContext())) {
            return mHexPage;
        }
        return null;
    }
    ...
    private void setOrder() {
        ...
        if(CalculatorSettings.hexPanel(mParent.getContext())) {
            Calculator.SmallPanel.HEX.setOrder(mCount);
            mCount++;
        }
        if(CalculatorSettings.advancedPanel(mParent.getContext())) {
            Calculator.SmallPanel.ADVANCED.setOrder(mCount);
            mCount++;
        }
        if(CalculatorSettings.functionPanel(mParent.getContext())) {
            Calculator.SmallPanel.FUNCTION.setOrder(mCount);
            mCount++;
        }
    }
